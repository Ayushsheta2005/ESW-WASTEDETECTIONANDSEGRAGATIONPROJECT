...
def _update_detection_history(recyclable_items, non_recyclable_items, hazardous_items):
    current_time = time.time()
    if 'detection_history' not in st.session_state:
        st.session_state['detection_history'] = []
    
    # Record detected items and their counts
    st.session_state['detection_history'].append({
        "timestamp": current_time,
        "Recyclable": len(recyclable_items),
        "Non-Recyclable": len(non_recyclable_items),
...
def _display_detected_frames(model, st_frame, image):
    ...
    for result in res:
        ...
        if new_classes != st.session_state['unique_classes']:
            ...
            _update_detection_history(recyclable_items, non_recyclable_items, hazardous_items)
